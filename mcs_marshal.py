...
def get_garbage_score(obj: dict, ops: dict[str, int]) -> int:
    score = 0
    code: bytes = obj.get('code', b'')
    names: list = obj.get('names', [])
    consts: list = obj.get('consts', [])
    varnames: list = obj.get('varnames', [])
    ...
    fast_ops = {ops.get(n) for n in ('STORE_FAST', 'LOAD_FAST', 'DELETE_FAST') if ops.get(n)}
    const_op = ops.get('LOAD_CONST')
    
    invalid_op_list = [op for name, op in ops.items() 
                       if name.startswith(('INPLACE', 'BINARY', 'ROT'))]
    ...
    try:
        while i < code_len:
            ...
            if op >= 93:
                ...
                if op == const_op:
                    if arg >= len(consts): score += 60
                elif op in name_ops:
                    if arg >= len(names): score += 60
                elif op in fast_ops:
                    if arg >= len(varnames): score += 60
    ...
    nop_op = ops.get('NOP', 9)
    nop_count = code.count(nop_op)
    if code_len > 10 and (nop_count / code_len) > 0.3:
        score += 40
...
class McsMarshal:
    ...
    def r_code_object(self, tag: int) -> dict:
        ...
        if self.remove_garbage:
            ...
            if get_garbage_score(obj, ops) > 100:
